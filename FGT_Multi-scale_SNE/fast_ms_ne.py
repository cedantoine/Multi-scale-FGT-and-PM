...
def fmssne(X_hds, n_components=2, init='pca', rand_state=None, nit_max=30, gtol=10.0**(-5), ftol=2.2204460492503131e-09, maxls=50, maxcor=10, fit_U=True, bht=0.45, fseed=1):
    ...
    cython_implem.fmssne_implem(X_hds_1D, X_lds, N, M, n_components, True, 1, fit_U, bht, nit_max, gtol, ftol, maxls, maxcor, 1, fseed, True)
    # Reshaping the LDS
...
def viz_2d_emb(X, vcol, tit='', fname=None, f_format=None, cmap='turbo', sdot=20, marker='o', a_scat=0.8, edcol_scat='face', stit=15, lw=2.0):
    """
...
if __name__ == '__main__':
    ...
    for idx_data, data_t in enumerate(L_data):
        ...
        if big_data:
            ...
            print('Fast multi-scale SNE will be applied.')
        ...
        print('- Applying (PCA) fast multi-scale SNE on the data set to obtain a {dim_LDS}-D embedding'.format(dim_LDS=dim_LDS))
        if data_name == 'Blobs':
        ...
        # pca =  sklearn.manifold.TSNE(n_components=2, method='exact', perplexity=30.0)
        # pca = sklearn.manifold.MDS(n_components=2)
        ...
        # X_ld_fmssne = pca.fit_transform(X_hds)

        data = scipy.io.loadmat('.//LD_pos.mat') 
        X_ld_fmssne = data['lastMatrix']
        ...
        if plot_emb:
            ...
            viz_2d_emb(X=X_ld_fmssne, vcol=labels, fname=None, f_format=None, sdot=10)
            print('===')
            print('===')
            print('===')

            # tit='LD embedding PCA ({data_name} data set)'.format(data_name=data_name)
