...
cdef inline lbfgsfloatval_t fmssne_evaluate(void* instance, const lbfgsfloatval_t* x, lbfgsfloatval_t* g, const int n, const lbfgsfloatval_t step):
    ...
    for rs in range(popt.n_rs): #n_rs = Number of random samplings
        memset(popt.sa[rs], 0, popt.sbsa) #sa = Suppl. attributes for the tree, when evaluating the gradient ; sbsa = Size in bytes of sa[rs] 
    # Intermediate variable
    cdef const double* xi

    # Computing the necessary LD distances (euclidean here)
    idx = 0
    ...
    idxsa = 0
    # For each data point
    for i in range(popt.N): #from here on BH ############################################### N = number of data points
        ...
        for rs in range(popt.n_rs): #n_rs = Number of random samplings
            ...
            for j in range(popt.nnn[rs][i]): #nnn = Number of considered neighbors for each data point
...
cpdef inline void fmssne_implem(double[::1] X_hds, double[::1] X_lds, int N, int d_hds, int n_components, bint cperp, int n_rs, bint fit_U, double ms_thetha, int nit_max, double gtol, double ftol, int maxls, int maxcor, int L_min, int rseed, bint sym_nn):
    ...
    for k in range(1, L-1, 1):
        ...
        if fit_U:
            popt.p_h_rs = &p_h_rs[h] #ici h normalement
            popt.t_h_rs = &t_h_rs[h] #ici h normalement
        else:
            popt.p_h = &p_h[h]
            popt.t_h = &t_h[h]
        popt.sbsa += K_star
...
"""
cpdef inline void gradient_test(int N):

    #on va faire avec un exemple de 4 pts, je vais définir toutes les variables dans la fct (hardcode) , sinon ca va être chiant

    cdef int i, j

    cdef double* g_bis = <double*> PyMem_Malloc(2*N * sizeof(double))
    for i in range(2*N):
        g_bis[i] = 0.0
    
    cdef double temp = 1.0

    cdef double* x_lds = <double*> PyMem_Malloc(2*N * sizeof(double))

    for i in range(2*N):
        x_lds[i] = temp
        temp += 2.0

    cdef double** laplacian = <double**> PyMem_Malloc(N * sizeof(double*))
    for i in range(N):
        laplacian[i] = <double*> PyMem_Malloc(2 * sizeof(double))
    ...
    cdef int** index_laplacian = <int**> PyMem_Malloc(N * sizeof(int*))
    for i in range(N):
        index_laplacian[i] = <int*> PyMem_Malloc(2 * sizeof(int))
    
    index_laplacian[0][0] = 1
    index_laplacian[0][1] = 3
    index_laplacian[1][0] = 0
    index_laplacian[1][1] = 2
    index_laplacian[2][0] = 0
    index_laplacian[2][1] = 3
    index_laplacian[3][0] = 1
    index_laplacian[3][1] = 2

    cdef double* diag_laplacian = <double*> PyMem_Malloc(N * sizeof(double))

    for i in range(N):
        diag_laplacian[i] = 8.0

    cdef double* SX = <double*> PyMem_Malloc(N * sizeof(double))
    ...
    cdef double* S_x_1 = <double*> PyMem_Malloc(N * sizeof(double))

    for i in range(N):
        S_x_1[i] = 2.0
    
    cdef double* S_x_2 = <double*> PyMem_Malloc(N * sizeof(double))
    ...
    for i in range(N):
        for j in range(2):
            g_bis[index_laplacian[i][j]*2] -= 4 * x_lds[2*i] * laplacian[i][j]
            #printf("Index %d - %f * %f\n",index_laplacian[i][j]*2, x_lds[2*i], laplacian[i][j])
            g_bis[index_laplacian[i][j]*2+1] -= 4 * x_lds[2*i+1] * laplacian[i][j]
            #printf("Index %d - %f * %f\n",index_laplacian[i][j]*2+1, x_lds[2*i+1], laplacian[i][j])
            #sleep(20)

    for i in range(N):
        g_bis[2*i] += (4 * x_lds[2*i] * diag_laplacian[i]) - (4 * x_lds[2*i] * SX[i] / c) + (4 * S_x_1[i] / c)
        g_bis[2*i+1] += (4 * x_lds[2*i+1] * diag_laplacian[i]) - (4 * x_lds[2*i+1] * SX[i] / c) + (4 * S_x_2[i] / c)
